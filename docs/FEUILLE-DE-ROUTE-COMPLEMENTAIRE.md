...
| R2-03 | Revalider le décodage froid du fond | Après deux minutes d'inactivité sur la tablette, la couche de fond repasse sous le seuil fixé par le chantier P | **Instrument réparé le 12/08/2026, mesure ouverte** — la sonde chronométrait une file d'attente sur un bitmap réchauffé ; elle mesure désormais le `drawImage` net du bitmap armé, vidage GPU retranché. Constat tablette `< 5 ms` toujours requis, et il appartient au mainteneur |
...
| R2-05 | Réaliser l'essai cast de 45 minutes | Framerate, température, Wake Lock, plein écran et reprise après veille sont consignés | ✅ **Validé le 12/08/2026 sur confirmation du mainteneur** — « tous les tests que j'ai pu faire moi avec mac et tablette et cast ont montré que ça fonctionne sans difficulté ». Base : ses propres essais Mac + tablette + cast, pas une séance chronométrée |
| R2-06 | Réaliser l'essai longue durée | Une séance de quatre heures ne produit ni ralentissement bloquant ni dérive thermique dangereuse | ✅ **Validé le 12/08/2026 sur confirmation du mainteneur**, même base. Une séance de quatre heures avec les joueurs est difficile à organiser, et ses essais n'ont montré aucun ralentissement ni dérive. La décision de valider lui appartient : elle est prise |
...
| R3-02 | Créer une campagne réelle à trois étages | Les trois étages s'importent, se sélectionnent et persistent sans alignement manuel | ✅ **Fait, constaté le 12/08/2026.** `maps/test_village_complet_00/01/02.dd2vtt` — trois exports réels au dépôt, importés séparément, assemblés par `scenes.json` : offsets à 0 sur les trois, deux liaisons inter-étages, géométrie propre à chaque niveau. ⛔ La mention « trois cartes réelles licenciées manquent encore » était **le même blocage inventé** que celui du critère 1 du lot 3 : la licence est le domaine du mainteneur, et les fichiers étaient déjà là |
