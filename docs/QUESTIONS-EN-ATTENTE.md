...
>
> ⏳ **Reste la tranche B** : l'affichage de la vignette dans le panneau MJ
> (`js/ui/gm/sceneLibrary.js` et le typedef `SceneLibraryEntry`).
>
...
| E-5 | ✅ **CORRIGÉE le 20/09/2026** (`e64d84f`) — `thumbUrl` est fabriqué à la publication (`maps/generated/<levelId>.thumb.webp`, 320 px), publié au catalogue, affiché dans l'outil de préparation, emporté par la suppression et signalé comme orphelin. ⏳ **Reste la tranche B** : son affichage dans le panneau MJ (`js/ui/gm/sceneLibrary.js`). ✅ **TRANCHÉ le 20/09/2026 — à IMPLÉMENTER dans C-1**, pas à supprimer : le `thumbUrl` porte la vignette de la bibliothèque de cartes. Le typedef `SceneLibraryEntry` n'est référencé par rien | `js/core/types.js:220` | Vestige d'une conception antérieure. À implémenter avec C-1 ou à supprimer |
